...
Let's jump in and use SignalFlow to preprocess and generate ERP tracings from a VEP dataset. We will have a few objectives:

- Arrange the files for optimized processing 
- Preprocess the files using the popular HAPPE pipeline
- Output 3 data tables
    - A table of EEG characteristics (i.e. original length)
    - A table of preprocessing results (i.e., # of epochs)
    - A table of amplitudes to generate ERPs
- Modify a preprocessing step to observe effect on ERPs
