...
filename = which('HAPPE_v3_BC_W_HBCD_ALLrun_DIN.m');
[filepath, ~, ~] = fileparts(filename);
happeDir = strcat(filepath, '\HAPPE_preprocessing_pipeline\HAPPE-master');
%happeDir = 'Z:\HBCD\Official_Pilot\HAPPE_preprocessing_pipeline\HAPPE-master' ;
...
filename = which('HAPPE_v3_BC_W_HBCD_ALLrun_DIN.m');
[filepath, ~, ~] = fileparts(filename);
srcDir = strcat(filepath, '\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE')
%srcDir = 'Z:\HBCD\Official_Pilot\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE'
cd (srcDir) ;
%% DETERMINE IF REPROCESSING DATA
%[reprocess, ranMuscIL, rerunExt] = isReprocessed() ; %MA
...
ver = '3_1_0' ;
%[preExist, params, changeParams] = isPreExist(reprocess, ver) ; %MA

preExist = 1
changeParams = 0
filename = which('HAPPE_v3_BC_W_HBCD_ALLrun_DIN.m');
[filepath, ~, ~] = fileparts(filename);
paramFile = strcat(filepath, '\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG_700_DIN.mat')
%paramFile = 'Z:\HBCD\Official_Pilot\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG_700_DIN.mat';
load(paramFile)


%Z:\Dropboxes\mantunez\Laurel\HAPPE ER V3 BugFixed_2\Raw OIT MMN data_BC_W\input_parameters\inputParameters_19-09-2022.mat
%paramFile = 'Z:\Dropboxes\mantunez\Laurel\HAPPE ER V3 BugFixed_2\Raw OIT MMN data\input_parameters\inputParameters_19-09-2022.mat';

%Z:\Dropboxes\mantunez\HBCD\HAPPE_BC_W_Processing_MMN\input_parameters\inputParameters_05-10-2022.mat
%Z:\Dropboxes\mantunez\HBCD\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE.mat
%Z:\Dropboxes\mantunez\HBCD\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG.mat
%Z:\Dropboxes\mantunez\HBCD\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG_800.mat
%Z:\HBCD\Official_Pilot\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG_800_DIN.mat
%Z:\HBCD\Official_Pilot\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG_700_DIN.mat
%Z:\HBCD\Official_Pilot\HAPPE_preprocessing_pipeline\HAPPE_BC_W_Processing_FACE\input_parameters\params_BC_W_HBCD_FACE_noECG_700_DIN_200mv.mat
...
%outerlayer_channel = 
% {'E17' 'E38' 'E43' 'E44' 'E48' 'E49' 'E113' 
% 'E114' 'E119' 'E120' 'E121' 'E125' 'E126' 'E127' 
% 'E128' 'E56' 'E63' 'E68' 'E73' 'E81' 'E88' 'E94' 'E99' 'E107'}; % list of channels
%  'ECG'
...
%                         if isfield(EEGraw, 'EEG'); EEGraw = EEGraw.EEG; end
                    ...
                    elseif params.loadInfo.inputFormat == 3
                      %  EEGraw = load('-mat', FileNames{currFile}) ;
                      %  if isfield(EEGraw, 'EEG'); EEGraw = EEGraw.EEG; end
                      EEG = pop_loadset([srcDir filesep FileNames{currFile}]); %LY changed load function
                      EEGraw = EEG;
                      
                      % Label OIT MMN            
%                      cd('R:\Projects\MADE pipeline project\Waveletting\scripts'); Edit_events_MMN_new_PC(); %Martin % This access to Ludwig      
%                          cd(srcDir) ;  %Martin%Hard Code for Rosalind OIT
%                 % ---------------------------------------------------------
                % .CDT FILES
...
%DIN Labeling Script
%Lydia Yoder
%Updated 1/9/23: Added code to label the "fourth" condition in the FACE
%task for files collected before the task was updated. For FACE, it only accurately
%labels the stm+ and DIN3 conditions, so ignore all other flag's condition labels


for i = 1:length(EEG.event)
    EEG.event(i).Condition = num2str(EEG.event(i).Condition);
end
...
if contains(EEG.filename, 'MMN')
    ...
    elseif contains(EEG.filename, 'RS')
        task = 'RS';
    elseif contains(EEG.filename, 'Baseline')
        task = 'RS';
...
%FACE
if strcmp(task, 'FACE')
    if numel(find(strcmp({EEG.event.type}, 'bgin'))) ~= 0 %if there are bgin flags, delete DIN3 from before the first bgin
        bgin_inds = find(strcmp({EEG.event.type}, 'bgin'));
        bad_dins = strcmp({EEG.event(1:bgin_inds(1)).type}, 'DIN3');
        if sum(bad_dins)>0
            EEG.event(bad_dins) = [];
        end
    ...
    for d =1:length(dins)
        ...
        if d <=100
           ...
           if strcmp(EEG.event(dins(d)-1).Condition, '1')
               ...
               EEG.event(dins(d)-2).Condition = upright_condition_b1;
           end
        else
...
if strcmp(task, 'MMN')
    if numel(find(strcmp({EEG.event.type}, 'bgin'))) ~= 0 %if there are bgin flags, delete DIN3 from before the first bgin
        bgin_inds = find(strcmp({EEG.event.type}, 'bgin'));
        bad_dins = strcmp({EEG.event(1:bgin_inds(1)).type}, 'DIN2');
        if sum(bad_dins)>0
            EEG.event(bad_dins) = [];
        end
        dins = find(strcmp({EEG.event.type}, 'DIN2'));
    else
        dins = find(strcmp({EEG.event.type}, 'DIN2'));
    end
    ...
    if num_stm  < num_din
         for w = 1:length(dins)
              if ~strcmp({EEG.event(dins(w)-1).type}, 'stms')
                            EEG.event(dins(w)).type = 'EXTRA_DIN';
              end
         end
    end   
