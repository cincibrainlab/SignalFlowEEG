...
% Author:  Elliot Layden, The University of Chicago, 2017
    ...
    % De-mean Columns:
    X = bsxfun(@minus,X,sum(X,1, 'omitnan')./size(X,1)); 
    Y = bsxfun(@minus,Y,sum(Y,1, 'omitnan')./size(Y,1));
    
    % Normalize by the L2-norm (Euclidean) of Rows:
    X = X.*repmat(sqrt(1./max(eps,sum(abs(X).^2,1, 'omitnan'))),[size(X,1),1]); 
    Y = Y.*repmat(sqrt(1./max(eps,sum(abs(Y).^2,1, 'omitnan'))),[size(Y,1),1]);
